...
fn perform_replacements_in_file(
    ...
    // Sort the items so they're in order - ripgrep should give them to us in order anyway but we sort them here to future-proof against any changes.
    // NOTE: we're sorting by the offset here with the assumption that no two Match items within one file will have the same offset.
    items.sort_unstable_by_key(|i| i.offset());

    // Iterate over the items in _reverse_ order -> this is so offsets can stay the same even though we're making changes to the string.
...
mod tests {
    ...
    fn it_performs_replacements_files_with_non_utf8_paths_unix() {
        ...
        // Here, the values 0x66 and 0x6f correspond to 'f' and 'o' respectively. The value 0x80 is a lone continuation byte, invalid in a UTF-8 sequence.
    ...
    // The following are generated with: printf "<BOM>%s" $(printf "foo bar baz\n...\nbaz foo bar\n...\nbar baz foo" | iconv -f UTF8 -t <ENCODING> | xxd -p -c 128)
    ...
    // The following are generated with: //   printf "<BOM>%s" $(printf "RUST bar baz\n...\nbaz RUST bar\n...\nbar baz RUST" | iconv -f UTF8 -t <ENCODING> | xxd -p -c 128)
    ...
    // The following are generated with: //   printf "<BOM>%s" $(printf "A bar baz\n...\nbaz A bar\n...\nbar baz A" | iconv -f UTF8 -t <ENCODING> | xxd -p -c 128)
    ...
    // The following are generated with: //   printf "<BOM>%s" $(printf "🦀 bar baz\n...\nbaz 🦀 bar\n...\nbar baz 🦀" | iconv -f UTF8 -t <ENCODING> | xxd -p -c 128)
    ...
    // The following are generated with: //   printf "%s" $(printf "¯\_(ツ)_/¯ bar baz\n...\nbaz ¯\_(ツ)_/¯ bar\n...\nbar baz ¯\_(ツ)_/¯" | iconv -f UTF8 -t <ENCODING> | xxd -p -c 128)
