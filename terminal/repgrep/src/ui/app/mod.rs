...
pub struct App {
    ...
    /// If the user passed a regular expression with a capturing group, then this will be set to indicate that we should use the capturing group when performing replacements.
...
impl App {
    ...
    /// Consume the app and return `ReplacementCriteria`. This will return an `Err` if the app wasn't in a state where the user had entered any replacement text.
