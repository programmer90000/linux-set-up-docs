...
impl App {
    ...
    fn draw_main_view<B: Backend>(&mut self, f: &mut Frame<B>, r: Rect) {
        ...
        // For performance with large match sets, we only send a single "window"'s worth of lines to the terminal. For all our calculations, we use a line count (to generate the list, define the window, determine the position of the indicator, etc) and only when we send it to the rendering library
        ...
        // iterate over all our items and collect only those that will be in the visible window region of the list (skipping all the others)
