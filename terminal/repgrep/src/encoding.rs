...
pub fn get_encoder(bytes: &[u8], rg_encoding: &RgEncoding) -> (Option<Bom>, EncodingRef) {
    ...
    let encoder = bom
        ...
        .or_else(|| {
            ...
            if confidence > 0.80 {
                // If we pass "ascii" to `encoding_from_whatwg_label` then it will default to using the "windows-1252" encoding. However, this may be confusing as most users are more familiar with ASCII encodings and may be unaware that "windows-1252" is an ASCII compatible encoding.
                if encoding == "ascii" {
                    // And, instead of forcing ASCII encoding here, we default to UTF8 which is far more common and is also backwards compatible with ASCII. This allows multi-code-point graphemes to work, as well as emoji and such.
