...
pub struct RgArgs {
    /// All the regular expressions that were passed. We need these since we perform matching ourselves in certain situations when rendering the TUI.
    pub patterns: Vec<String>,
    /// Any encoding that was passed - we want to force the same encoding that ripgrep uses when we perform any replacements ourselves.
    pub encoding: Option<String>,
    /// Whether fixed strings was enabled - means we only need to substring search rather than regular expression searching.
...
impl RgArgs {
    ...
    fn parse_rg_unknown_arg(
        ...
        let next_is_flag = parser
            ...
            // if `try_raw_args` failed, then we're passing something with an optional value, so that's not a flag
            .unwrap_or(false);
    ...
    fn parse_rg_args_impl(mut parser: Parser) -> Result<RgArgs> {
        ...
        // as per ripgrep's documentation: When -f/--file or -e/--regexp is used, then ripgrep treats all positional arguments as files or directories to search.
        ...
        while let Some(arg) = parser.next()? {
            match arg {
                ...
                // capture help to display our help. important to capture these since they make `rg` not output JSON!
                Short('h') | Long("help") => {
                    print_help();
                    process::exit(0);
                }
