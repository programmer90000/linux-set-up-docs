...
pub(crate) struct HunkHeader {
    ...
    /// One-based start lines of one or more old sections + one new section. This vector will always have at least two entries, and mostly it will be exactly two.
    starts: Vec<usize>,

    /// Number of lines in one or more old sections + one new section. This vector will always have at least two entries, and mostly it will be exactly two.
...
impl HunkHeader {
    /// Parse a hunk header from a line of text. Returns `None` if the line is not a valid hunk header.
