...
impl Drop for LineCollector {
    fn drop(&mut self) {
        ...
        // Tell the consumer thread to drain and quit. Sending an empty string like this is the secret handshake for requesting a shutdown.
...
impl LineCollector {
    pub fn new<W: io::Write + Send + 'static>(
        ...
        // This is how many entries we can look ahead. An "entry" in this case being either a plain text section or an oldnew section.
        // Benchmark timings with different multipliers on an 8 logical cores machine with a 6.6M lines / 208MB diff:
    ...
    /// The line parameter is expected *not* to end in a newline. Returns an error message on trouble.
    pub fn consume_line(
        &mut self,
        raw_line: &[u8],
    ...
    fn consume_line_internal(
        ...
        // Strip out incoming ANSI formatting. This enables us to highlight already-colored input.
        ...
        if line.starts_with('\\') {
            {
                // Store the "\ No newline at end of file" string however it is phrased in this particular diff. Note that this must be done before consuming it below so we know it's set before the consumer decides it wants to emit a copy. Otherwise we get a race condition and we don't want that.
