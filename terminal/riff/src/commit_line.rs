...
pub fn format_commit_line(line: &str, highlight_background: bool) -> String {
    ...
    if without_trailing_parenthesis.is_none() {
        // No final parenthesis, this is weird, fall back to showing everything in yellow
        return format!("{header}{line}{NORMAL}");
    }
...
fn format_commit_part(part: &str, current_branch: &Option<String>) -> String {
    if part.starts_with("tag: ") {
        // Implicitly yellow since both the commas and the surrounding parentheses are also yellow.
...
fn compute_current_branch(candidates: &Vec<&str>) -> Option<String> {
    // If we find multiple options, pick the one with the lowest number of slashes
...
mod tests {
    ...
    #[test]
    // Verify that the blue background goes all the way from the start of the string to its end
