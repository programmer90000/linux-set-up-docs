...
(defstruct ert--stats
  ...
  ;; A map of test names (or the test objects themselves for unnamed tests) to indices into the `tests' vector.
  (test-map (assert nil) :type hash-table)
  ;; The results of the tests during this run, in order.
  (test-results (assert nil) :type vector)
  ;; The start times of the tests, in order, as reported by `current-time'.
  (test-start-times (assert nil) :type vector)
  ;; The end times of the tests, in order, as reported by `current-time'.
  ...
  (aborted-p nil)
  (current-test nil)
  ;; The time at or after which the next redisplay should occur, as a float.
  (next-redisplay 0.0))
...
;; The stats object of the current run, dynamically bound.  This is used for the mode line progress indicator.
...
(defun ert-run-or-rerun-test (stats test listener)
  ...
  (let ((ert--current-run-stats stats)
    ...
    ;; Call listener after setting/before resetting (ert--stats-current-test stats); the listener might refresh the mode line display, and if the value is not set yet/any more during this refresh, the mode line will flicker unnecessarily.
...
Signals an error if no test name was read."
  ...
  (let ((input (completing-read prompt obarray #'ert-test-boundp
    ...
    ;; completing-read returns an empty string if default was nil and the user just hit enter.
