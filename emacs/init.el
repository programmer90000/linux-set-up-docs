...
(defvar search-replace-sidebar-buffer-name "*Search And Replace*")
(defvar search-replace-input1 nil)
(defvar search-replace-input2 nil)
(defvar search-replace-width 35)

(defun search-replace-sidebar-create ()
  "Create the search and replace sidebar buffer."
  (let ((buffer (get-buffer-create search-replace-sidebar-buffer-name)))
    (with-current-buffer buffer
      (let ((inhibit-read-only t))
        (erase-buffer)

        ;; Content
        (widget-insert (propertize "Search For:" 'face 'font-lock-comment-face))
        (widget-insert "\n")
        (widget-insert "Text: ")
        (setq search-replace-input1
              (widget-create 'editable-field :size 20 :value "" :format "%v"))
        (widget-insert "\n\n")

        (widget-insert (propertize "Replace With:" 'face 'font-lock-comment-face))
        (widget-insert "\n")
        (widget-insert "Text: ")
        (setq search-replace-input2
              (widget-create 'editable-field :size 20 :value "" :format "%v"))
        (widget-insert "\n\n")
...
(defun search-replace-sidebar-show ()
  ...
  (let ((buffer (search-replace-sidebar-create)))
    (display-buffer-in-side-window
     ...
     `((side . left)
       (slot . 0)
       (window-width . ,search-replace-width)
       (window-parameters . ((no-delete-other-windows . t)))))))

(defun search-replace-sidebar-toggle ()
  "Toggle the search and replace sidebar."
  (interactive)
  (let ((sidebar-window (get-buffer-window search-replace-sidebar-buffer-name)))
    (if sidebar-window
        (quit-window nil sidebar-window)
      (search-replace-sidebar-show))))

;; Resize functions
(defun search-replace-sidebar-increase-width ()
  "Increase sidebar width by 5 columns."
  (interactive)
  (setq search-replace-width (+ search-replace-width 5))
  (search-replace-sidebar-refresh))

(defun search-replace-sidebar-decrease-width ()
  "Decrease sidebar width by 5 columns (minimum 20)."
  (interactive)
  (setq search-replace-width (max 20 (- search-replace-width 5)))
  (search-replace-sidebar-refresh))

(defun search-replace-sidebar-reset-width ()
  "Reset sidebar width to default 35 columns."
  (interactive)
  (setq search-replace-width 35)
  (search-replace-sidebar-refresh))
...
(add-hook 'widget-setup-hook
          (lambda ()
            (when (string= (buffer-name) search-replace-sidebar-buffer-name)
