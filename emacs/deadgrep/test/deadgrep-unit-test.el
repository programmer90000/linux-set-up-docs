...
(ert-deftest deadgrep-forward ()
  (with-temp-deadgrep-buf
   ...
   ;; Moving forward, when point is already on the last item, should not error.
   (goto-char (point-max))
   (deadgrep-forward)
...
(ert-deftest deadgrep-backward ()
  (with-temp-deadgrep-buf
   ...
   ;; Moving backward, when point is already on the first item, should not error.
   (goto-char (point-min))
   (deadgrep-backward)
   ...
   (equal
    (deadgrep--relevant-file-type
     ...
     '(("clojure" ("*.cljs" "*.clj"))
       ("py" ("*.py"))))
    '("clojure" ("*.cljs" "*.clj"))))
  ;; If there are multiple matches, take the match with the largest number of extensions.
  (should
   (equal
    (deadgrep--relevant-file-type
     "foo.ml"
     '(("ml" ("*.ml"))
       ("ocaml" ("*.ml" "*.mli"))))
    '("ocaml" ("*.ml" "*.mli"))))
  ;; If there are duplicates with different names, prefer the longer name.
  (should
   (equal
    (deadgrep--relevant-file-type
     "foo.md"
