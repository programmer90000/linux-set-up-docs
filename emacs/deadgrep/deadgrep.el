...
(defun deadgrep--insert-output (output &optional finished)
  ...
  (let ((inhibit-read-only t)
        ...
        prev-line-num)
    ;; Process filters run asynchronously, and don't guarantee that OUTPUT ends with a complete line. Save the last line for later processing.
    (unless finished
    ...
    (save-excursion
      ...
      (dolist (line lines)
        (cond
         ;; Ignore blank lines.
         ((s-blank? line))
         ;; Lines of just -- are used as a context separator when calling ripgrep with context flags.
         ((string= line "--")
          (let ((separator "--"))
            ;; Try to make the separator length match the previous line numbers.
            (when prev-line-num
              (setq separator
         ...
         ;; If we have a warning or don't have a color code, ripgrep must be complaining about something (e.g. zero matches for a glob, or permission denied on some directories).
         ((or
...
(defun deadgrep--process-filter (process output)
  ;; Searches may see a lot of output, but it's really useful to have a snippet of output when debugging. Store the first output received.
...
(defun deadgrep--relevant-file-type (filename types-and-globs)
  ...
  (let (;; Find all the items in TYPES-AND-GLOBS whose glob match FILENAME.
        (matching (-filter (-lambda ((_type globs))
                             (deadgrep--matches-globs-p filename globs))
                           types-and-globs)))
    (->> matching
         ;; Prefer longer names, so "markdown" over "md" for the type name.
         (-sort (-lambda ((type1 _) (type2 _))
                  (< (length type1) (length type2))))
         ;; Prefer types with more extensions, as they tend to be more common languages (e.g. 'ocaml' over 'ml').
         (-sort (-lambda ((_ globs1) (_ globs2))
                  (< (length globs1) (length globs2))))
         ;; But prefer elisp over lisp for .el files.
         (-sort (-lambda ((type1 _) (type2 _))
                  ;; Return t if we're comparing elisp with lisp, nil otherwise.
  ...
  (let*
      ((initial-value
        (cond
         ...
         ;; If the initial file had a file name of the form foo.bar, offer *.bar as the initial glob.
...
(defun deadgrep--button (text type &rest properties)
  ;; `make-text-button' mutates the string to add properties, so copy TEXT first.
...
to obtain ripgrep results."
  ;; We put the extra arguments first so that later arguments will override them, preventing a user from accidentally breaking ripgrep by specifying --heading, for example.
  (let ((args (copy-sequence deadgrep-extra-arguments)))
    ...
    (if deadgrep--skip-if-vcs-ignore
        ;; By default, ripgrep searches .git even when it's respecting .gitignore, if --hidden is set. Ignore .git when we're using .gitignore.
        (push "--glob=!/.git" args)
...
Returns a copy of REGEXP with properties set."
  ...
  (let ((metachars
         ...
         '(?\( ?\) ?\[ ?\] ?\{ ?\} ?| ?. ?+ ?* ?? ?^ ?$))
        ;; Characters that have special regexp meaning when preceded with a backslash. This includes things like \b but not things like \n.
        (escape-metachars
         '(?A ?b ?B ?d ?D ?p ?s ?S ?w ?W ?z))
        (prev-char nil))
    ;; Put the standard search term face on every character individually.
    (dotimes (i (length regexp))
      (put-text-property
       i (1+ i)
       'face 'deadgrep-search-term-face
       regexp))
    ;; Put the metacharacter face on any character that isn't treated literally.
...
Returns a list ordered by the most recently accessed."
  (--filter (with-current-buffer it
            ...
            ;; `buffer-list' seems to be ordered by most recently visited first.
            (buffer-list)))
...
(defun deadgrep--buffer (search-term directory initial-filename)
  ...
  (let* ((buf-name (deadgrep--buffer-name search-term directory))
    ...
    (if buf
        ;; There was already a buffer with this name. Reset its search state.
        (with-current-buffer buf
          (deadgrep--stop-and-reset))
      ;; We need to create the buffer, ensure we don't exceed `deadgrep-max-buffers' by killing the least recently used.
      (progn
        (when (numberp deadgrep-max-buffers)
          (let* ((excess-buffers (-drop (1- deadgrep-max-buffers)
                                        (deadgrep--buffers))))
            ;; Kill buffers so we have one buffer less than the maximum before we create a new one.
    ...
    (with-current-buffer buf
      ...
      (let ((inhibit-read-only t))
        ;; This needs to happen first, as it clobbers all buffer-local variables.
        (deadgrep-mode)
        (erase-buffer)
...
this path."
  (let* ((initial-buffers (buffer-list))
         ...
         (find-file-hook nil)
         ;; If we end up opening a buffer, don't bother with file variables. It prompts the user, and we discard the buffer afterwards anyway.
         (enable-local-variables nil)
         ;; Bind `auto-mode-alist' to nil, so we open the buffer in `fundamental-mode' if it isn't already open.
         (auto-mode-alist nil)
         ;; Use `find-file-noselect' so we still decode bytes from the underlying file.
...
underlying file."
  ;; We should never be called outside an edit buffer, but be defensive. Buggy functions in change hooks are painful.
        ...
        (with-current-buffer buf
          (save-mark-and-excursion
            (save-restriction
              ...
              (delete-char (- length))
              (insert inserted)))
          ;; If we weren't visiting this file before, just save it and close it.
          (when opened
            (basic-save-buffer)
            (kill-buffer buf)))))))
...
\\{deadgrep-edit-mode-map}"
  ...
  ;; We deliberately don't use `define-derived-mode' here because we  want to check the previous value of `major-mode'. Initialise the major mode manually.
  (delay-mode-hooks
...
(defun deadgrep--visit-result (open-fn)
  ...
  (let* ((pos (line-beginning-position))
    ...
    (when file-name
      ...
      ;; Show an arrow next to the last result viewed. This is consistent with `compilation-next-error-function' and also useful with `deadgrep-visit-result-other-window'.
      ...
      (when line-number
        ;; If point was on the line number rather than a specific position on the line, go the first match. This is generally what users want, especially when there are long lines.
        ...
        (-let [destination-pos (deadgrep--buffer-position
          ...
          ;; Put point on the position of the match, widening the buffer if necessary.
          (when (or (< destination-pos (point-min))
                    (> destination-pos (point-max)))
            (widen))
          (goto-char destination-pos)

          ;; Temporarily highlight the parts of the line that matched the search term.
...
(defun deadgrep--interrupt-process ()
  ...
  (-when-let (proc (get-buffer-process (current-buffer)))
    ...
    ;; Wait for the process to terminate, so we know that `deadgrep--process-sentinel' has been called.
  ...
  (let ((pos (point)))
    ...
    (while (and (not (deadgrep--item-p pos))
      ...
      (if forward-p
          (cl-incf pos)
        (cl-decf pos)))
    ;; Regardless of direction, ensure point is at the beginning of the item.
    (while (and (if forward-p
                    (< pos (point-max))
                  (> pos (point-min)))
                (deadgrep--item-p (1- pos)))
      (cl-decf pos))
    ;; If we reached an item (we aren't at the first/last item), then go to it.
        ...
        (progn
          ...
          (unless forward-p
            (while (eq (get-text-property (point) 'face)
              ...
              (backward-char))
            (forward-char)))
      ;; Don't move point beyond the last match. However, it's still useful to signal that we're at the end, so users can use this command with macros and terminate when it's done.
      (beginning-of-buffer
       (goto-char start-pos)
...
(defun deadgrep--stop-and-reset ()
  ...
  ;; Stop the old search, so we don't carry on inserting results from the last thing we searched for.
  (deadgrep--interrupt-process)

  (let ((inhibit-read-only t))
    ;; Reset UI: remove results, reset items hidden by TAB, and arrow position.
    (erase-buffer)
...
(defun deadgrep-restart ()
  ...
  (interactive)
  ;; If we haven't started yet, start the search if we've been called by the user.
  ...
  (let ((start-point (point))
    ...
    ;; If the point was in the heading, ensure that we restore its position.
  ...
  (let ((root default-directory)
    ...
    (when project
      (cond ((and (fboundp 'project-root)
             ...
             ;; `project-root' was added in Emacs 28, but projectile defines it unconditionally.
...
don't actually start the search."
  ...
  (let* ((dir (or directory
    ...
    (with-current-buffer buf
      ...
      ;; If we have previous search settings, apply them to our new search results buffer.
      (when last-results-buf
      ...
      (if current-prefix-arg
          ;; Don't start the search, just create the buffer and inform the user how to start when they're ready.
          (progn
  ...
  (let ((direction (> arg 0)))
    ...
    (while (and
      ...
      (if direction
          (forward-line 1)
        (forward-line -1))
      ;; If we are on a specific result (not a heading), we have a line number.
