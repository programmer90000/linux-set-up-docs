...
See also: `-unzip'."
  ...
  ;; For backward compatibility, return a list of dotted pairs if two arguments were provided.
...
(defun dash--match-cons (match-form source)
  ...
  (let ((s (dash--match-make-source-symbol))
    ...
    (while (and (consp m)
      ...
      (setq n (1+ n)) (!cdr m))
    (cond
     ;; when we only have one pattern in the list, we don't have to create a temporary binding (--dash-source--) for the source and just use the input directly
     ((and (consp m)
           (not (cdr m)))
     ...
     ;; this is the only entry-point for dash--match-cons-1, that's why we can't simply use the above branch, it would produce infinite recursion
     (t
  ...
  (let (test len freqs)
    (cond ((null list))
          ((and (setq test (dash--hash-test-fn))
                (> (setq len (length list)) dash--short-list-length))
           (let ((ht (make-hash-table :test test :size len)))
             ;; Share structure between hash table and returned list. This affords a single pass that preserves the input order, conses less garbage, and is faster than a second traversal (e.g., with `maphash').
             (--each list
...
Part I, Addison-Wesley, 2011, § 7.2.1.2, p. 319."
  ...
  (let* ((l n)
         ...
         (al (aref array n))
         (aj al))
    ;; L2. [Find j]. Decrement j until a[j] < a[j+1].
    (while (and (<= 0 j)
...
in LIST, as returned by `-frequencies'."
  (let (;; Distinct items in `list', aka the cars of `freqs'.
    ...
    (--each freqs
      ...
      ;; Populate `idxs' with as many copies of each `it-index' as there are corresponding duplicates.
      (dotimes (_ (cdr it))
...
`-compare-fn' if that is non-nil."
  (declare (important-return-value t))
  (cond ((null list) (list ()))
        ;; Optimization: a traversal of `list' is faster than the round trip via `dash--uniq-perms' or `dash--multi-perms'.
        ((dash--numbers<= list)
    ...
    (cond ((null (or list1 list2)))
          ...
          ((and (setq test (dash--hash-test-fn))
           ...
           (let ((ht1 (make-hash-table :test test :size len1))
             ...
             ;; Move visited elements from `ht1' to `ht2'.  This way, if visiting all of `list2' leaves `ht1' empty, then all elements from both lists have been accounted for.
...
equivalent:
  ...
  (declare (pure t) (side-effect-free error-free))
  (lambda (&rest args)
    ;; This unrolling seems to be a relatively cheap way to keep the overhead of `mapcar' + `apply' in check.
    (cond ((cddr args)
...
`global-dash-fontify-mode'."
  ...
  (static-if (fboundp 'font-lock-flush)
      (font-lock-flush)
    (when font-lock-mode
      ;; Unconditionally enables `font-lock-mode' and is marked `interactive-only' in later Emacs versions which have `font-lock-flush'.
