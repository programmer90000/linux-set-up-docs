  ...
  (let ((window (or window (selected-window))))
    (or (split-window-sensibly window)
        (and (get-buffer-window neo-buffer-name)
             ...
             ;; If WINDOW is the only window on its frame (or only include Neo window) and is not the minibuffer window, try to split it vertically disregarding the value of `split-height-threshold'.
