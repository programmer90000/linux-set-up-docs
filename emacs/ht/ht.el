...
user-supplied test created via `define-hash-table-test'."
  ...
  (let ((h (ht-create test)))
    ;; the first key-value pair in an alist gets precedence, so we start from the end of the list:
...
;; Don't use `ht-set!' here, gv setter was assumed to return the value to be set.
