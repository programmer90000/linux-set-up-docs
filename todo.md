# TODO

| Task                                                           | Status |
|----------------------------------------------------------------|--------|
| Installation                                                   | ❌     |
| Enable sudo access for your account                            | ❌     |
| Enable apt                                                     | ❌     |
| Set theme                                                      | ❌     |
| Delete unneeded apps                                           | ❌     |
| Change application launcher view                               | ❌     |
| Configure application dashboard                                | ❌     |
| Clear panel                                                    | ❌     |
| Add widgets                                                    | ❌     |
| Add apps to application dashboard                              | ❌     |
| Set profile picture                                            | ❌     |
| Set desktop background                                         | ❌     |
| Change lock screen apperance                                   | ❌     |
| Add apps to dash                                               | ❌     |
| Add widgets to dash                                            | ❌     |
| Add power menu widget to dash                                  | ❌     |
| Add apps to desktop                                            | ❌     |
| Install custom fonts                                           | ❌     |
| Change sound device names                                      | ❌     |
| Set boot theme                                                 | ❌     |
| Set system settings                                            | ❌     |
| Replace `.bashrc` file in the home directory with mine         | ❌     |
| Replace `.bash_aliases` file in the home directory with mine   | ❌     |
| Replace `.bash_logout` file in the home directory with mine    | ❌     |
| Replace `.profile` file in the home directory with mine        | ❌     |
| Connect to GitHub from the terminal                            | ❌     |
| Install bat                                                    | ❌     |
| Install tree                                                   | ❌     |
| Install duf                                                    | ❌     |
| Install ncdu                                                   | ❌     |
| Install Gemini CLI                                             | ❌     |
| Install Visual Studio Code                                     | ❌     |
| Install Kdenlive Video Editor                                  | ❌     |
| Install FileZilla                                              | ❌     |
| Install Google Chrome                                          | ❌     |
| Install Mousepad text editor                                   | ❌     |
| Install NVM                                                    | ❌     |
| Install Microsoft Edge                                         | ❌     |
| Install Zoom                                                   | ❌     |
| Install VirtualBox                                             | ❌     |
| Install HandBrake                                              | ❌     |
| Install GitHub Desktop                                         | ❌     |
| Install Brave Web Browser                                      | ❌     |
| Install Android Studio                                         | ❌     |
| Install Docker and Docker Desktop                              | ❌     |
| Install KDE Spectacle                                          | ❌     |
| Install CopyQ Clipboard Manager                                | ❌     |
| Install Tesseract OCR Text Extractor                           | ❌     |
| Install VLC Media Player                                       | ❌     |
| Install Viewnior Image Viewer                                  | ❌     |
| Install Gimp Image Editor                                      | ❌     |
| Install Shutter Screen Ruler                                   | ❌     |
| Install Krename File Renamer                                   | ❌     |
| Install Qdirstat Disk Space Analyzer                           | ❌     |
| Install ThunderBird Email Client                               | ❌     |
| Install Hardinfo System Profiler                               | ❌     |
| Install Stacer                                                 | ❌     |
| Install Hstr                                                   | ❌     |
| Install Clamav Malware Scanner                                 | ❌     |
| Setup Brave                                                    | ❌     |
| Setup Microsoft Edge                                           | ❌     |
| Setup Google Chrome                                            | ❌     |
| Setup Dolphin File Manager                                     | ❌     |
| Setup Thunderbird                                              | ❌     |
| Setup Visual Studio Code                                       | ❌     |
| Setup Konsole                                                  | ❌     |
| Setup Libre Office                                             | ❌     |
| Setup Settings App                                             | ❌     |
| Setup Android Studio                                           | ❌     |
| Setup Kdenlive Video Editor                                    | ❌     |
| Setup Viewnior                                                 | ❌     |
| Setup Mousepad                                                 | ❌     |
| Setup VLC Media Player                                         | ❌     |
| Setup Sound Recorder                                           | ❌     |
| Setup Tweaks app                                               | ❌     |
| Setup Handbrake                                                | ❌     |
| Setup CopyQ Clipboard Manager                                  | ❌     |
| Setup GitHub Desktop                                           | ❌     |
| Setup LibreOffice                                              | ❌     |
| Setup VirtualBox                                               | ❌     |
| Setup ClamAV                                                   | ❌     |
| Setup Sound Recorder                                           | ❌     |
| Add `bash-scripts.md` to the home directory                    | ❌     |
| Add `check-disk-space.sh` to the home directory                | ❌     |
| Add `check-if-process-running.sh` to the home directory        | ❌     |
| Add `check-internet-connection.sh` to the home directory       | ❌     |
| Add `network-ping.sh` to the home directory                    | ❌     |
